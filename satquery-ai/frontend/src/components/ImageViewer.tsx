...
import { 
  Maximize, ZoomIn, ZoomOut, RotateCcw, Layers, Download, 
  Crosshair, Info, Eye, EyeOff, Radio, Compass, SlidersHorizontal 
} from 'lucide-react';
...
const ImageViewer: React.FC<ImageViewerProps> = ({ 
  ...
  return (
    <div className="h-full w-full bg-space-950 flex flex-col relative select-none overflow-hidden">
      ...
      {/* Technical HUD Overlay (Bottom and Coordinates) */}
      {showHUD && (
        <>
          {/* Top-Center Coordinate Bar */}
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 pointer-events-none hidden md:flex items-center space-x-3 px-3 py-1 bg-space-950/80 border border-space-700 rounded-xs backdrop-blur-sm text-[10px] font-mono text-hud-muted">
            <span className="text-emerald">CENTROID: 28.550°N, 77.050°E</span>
          ...
          <div className="absolute bottom-3 left-4 right-4 z-20 pointer-events-none flex items-center justify-between text-[10px] font-mono">
            ...
            {mode === 'OPTICAL + SAR' && (
              <div className="px-2.5 py-1 bg-space-950/85 border border-emerald/40 rounded-xs backdrop-blur-sm text-emerald font-semibold shadow-glow-sm pointer-events-auto">
                CROSS-MODAL CORROBORATION: 80% HIGH
              </div>
            )}
          </div>
        </>
      ...
      >
        ...
        >
          {/* 1. Bi-Temporal Change Mode Comparison */}
          {activeTab === 'CHANGE' && secondaryImage ? (
            <div className="w-[700px] border border-emerald/40 shadow-glow-sm relative">
            ...
            /* 2. Optical + SAR Multimodal Dual/Tri View */
            <div className="flex items-center gap-4">
              <div className="relative border border-space-700 bg-space-900 shadow-xl">
                ...
                <img src={primaryImage} className="w-[360px] h-[360px] object-cover" alt="Optical" draggable={false} />
              </div>
              ...
              <div className="relative border border-space-700 bg-space-900 shadow-xl">
                ...
                <img src={secondaryImage} className="w-[360px] h-[360px] object-cover" alt="SAR" draggable={false} />
              </div>
            </div>
          ) : (
            /* 3. Standard Imagery Display with SVG Evidence Overlays */
            <div className="relative border border-space-700 shadow-2xl bg-space-900">
              <img 
                src={primaryImage} 
                className="max-w-[700px] max-h-[700px] object-contain block" 
                alt="Satellite Viewer" 
                draggable={false} 
              />

              {/* Dynamic SVG Spatial Bounding Boxes */}
              {(activeTab === 'EVIDENCE' || focusedRegion) && (
                <svg className="absolute inset-0 w-full h-full pointer-events-none">
                  {/* Default evidence boxes if items exist */}
                      ...
                      return (
                        <g key={i}>
                          <rect 
                            ...
                            width={`${(x2 - x1) * 100}%`} 
                            height={`${(y2 - y1) * 100}%`} 
                            fill={isFocused ? "rgba(0, 255, 135, 0.25)" : "none"} 
                            stroke={isFocused ? "#00ff87" : "#00ff87"} 
                  ...
                  {/* Fallback demo box if no dynamic boxes are yet present */}
